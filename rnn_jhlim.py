...
def sgd(lr, tparams, grads, x, mask, y, cost):
    ...
    # for a mini-batch.
    gshared = [theano.shared(p.get_value() * 0.9, name='%s_grad' % k)
...
def pred_error(f_pred, prepare_data, input_dim, output_dim, stride, data, iterator, verbose=False):
    """
...
def build_lstm_pred_model(tparams, options, n_timesteps=1, input_dim=1, output_dim=1):
    trng = RandomStreams(1234)
    ...
    #def _step(m_, x_, h_, c_): # m_: mask, x_: input, h_: previous hidden, c_: previous hidden
    def _stepstep(h_, c_, y_): # m_: mask, h_: previous hidden, c_: previous hidden, *y_: raw inputs
    ...
    if output_dim == input_dim: #if output_dim > 1 and input_dim > 1:
        rval, updates = theano.scan(_stepstep,
                                    ...
                                    outputs_info=[dict(initial=h0, taps=[-1]), # h0.ndim = 2 will be preserved
                                                  ...
                                                  dict(initial=y0.reshape([n_samples, output_dimd]), taps=[-1])], # y0.ndim = 3 will become y_[0].ndim = 2
        ...
        if input_dim == 1: 
            rval, updates = theano.scan(_step,
                                    ...
                                    outputs_info=[dict(initial=h0, taps=[-1]), # h0.ndim = 2 will be preserved
                                                  dict(initial=c0, taps=[-1]), # h0.ndim = 2 will be preserved
                                                  dict(initial=y0.reshape([n_samples, output_dimd]), taps=y_taps)], # y0.ndim = 3 will become y_[0].ndim = 2
                                                                                  # usage of taps is tricky!! fuck!!
                                    name=_p(prefix, '_layers'),
                                    n_steps=n_timesteps)
        else: 
...
def build_rnn_pred_model(tparams, options, n_timesteps=1, input_dim=1, output_dim=1):
    trng = RandomStreams(1234)
...
def train_lstm(
    ...
    stride = 1, # stride in input 
    #n_words=10000,  # Vocabulary size
    pred_steps = 110, # number of frames to be predicted 
    ...
    try:
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                # add noise 
                # print "x.shape", x.shape
                x = x + noise_std * numpy.random.randn(x.shape[0], x.shape[1], x.shape[2]).astype(config.floatX)
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    use_noise.set_value(0.)
                    train_err = pred_error(f_pred, prepare_data, input_dim, output_dim, stride, train, kf)
                    valid_err = pred_error(f_pred, prepare_data, input_dim, output_dim, stride, valid,
                                           kf_valid)
                    test_err = pred_error(f_pred, prepare_data, input_dim, output_dim, stride, test, kf_test)
    ...
    train_err = pred_error(f_pred, prepare_data, input_dim, output_dim, stride, train, kf_train_sorted)
    valid_err = pred_error(f_pred, prepare_data, input_dim, output_dim, stride, valid, kf_valid)
    test_err = pred_error(f_pred, prepare_data, input_dim, output_dim, stride, test, kf_test)
    ...
    if model_options['encoder']=='lstm':
        f_predpred = build_lstm_pred_model(tparams, model_options, n_timesteps=pred_steps, input_dim=input_dim, output_dim=output_dim)
    elif model_options['encoder']=='rnn':
        f_predpred = build_rnn_pred_model(tparams, model_options, n_timesteps=pred_steps, input_dim=input_dim, output_dim=output_dim)
...
#        x = [test[0][0]] # single seq
        ...
        xx = x[:(50-input_dim)/stride+1, :, :]
        maskmask = mask[:(50-input_dim)/stride+1, :]
        yy = y[:(50-input_dim)/stride+1, :, :]
        ...
        #print preds_y.shape
        #print preds_y[-input_dim:].shape
        #print x.shape
        #print x[(50-input_dim)/stride+1,:,:].shape
        ...
        if model_options['encoder']=='lstm':
            ...
            predpred = f_predpred(preds_h[-1], preds_c[-1], [x[(50-input_dim)/stride+1,:,:]])

        elif model_options['encoder']=='rnn':
            preds_h = f_hiddens[0](xx, mask)

            #predpred = f_predpred(preds_h[-1], preds_y[-input_dim:])
            predpred = f_predpred(preds_h[-1], [x[(50-input_dim)/stride+1,:,:]])
          
        #print "preds_y.shape: ", preds_y.shape
        #print "predpred.shape", predpred.shape

        y_pred_total = numpy.concatenate((preds_y, predpred), axis=0)
         
        #print "y_pred_total.shape: ", y_pred_total.shape
        #print "y.shape: ", y.shape
        #print "predpred.shape: ", predpred.shape
        #print "y[(50-input_dim)/stride+1:,:,:]).shape: ", y[(50-input_dim)/stride+1:,:,:].shape
        ...
        if (epoch_pred is 0) and (is_plot_save is True):
            for t in xrange(len(test_index)):
                ...
                # Graph 1
                xxx = x[0,t,:].reshape(input_dim,1)
                for i in xrange(1, x.shape[0]):
                    xxx = numpy.concatenate((xxx, x[i,t,:].reshape(input_dim,1)), axis=0)
                
                yyy = y[0,t,:].reshape(input_dim,1)
                yyy_pred_total = y_pred_total[0,t,:].reshape(input_dim,1)
