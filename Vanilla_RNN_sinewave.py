...
if __name__ == "__main__":
    ...
    print seq_test[6,0:50,:]
    print targets[6,:,:]
    W_uh = theano.shared(value = model.W_uh.get_value(), name = 'W_uh')
    W_hh = theano.shared(value = model.W_hh.get_value(), name = 'W_hh')
    W_hy = theano.shared(value = model.W_hy.get_value(), name = 'W_hy')
    h0 = theano.shared(value = np.zeros((n_h, ), dtype = theano.config.floatX), name = 'h0')
    b_h = theano.shared(value = model.b_h.get_value(), name = 'b_h')
    b_y = theano.shared(value = model.b_y.get_value(), name = 'b_y')
    ...
    [h_t_val, y_t_val] = predict1(seq_test[6,0:50,:])

    def recurrent_fn2(h_tm1, y_tm1):
        h_t = T.tanh(T.dot(y_tm1, W_uh) + \
                              T.dot(h_tm1, W_hh) + \
                              b_h)
        y_t = T.dot(h_t, W_hy) + b_y
        return h_t, y_t
    ...
    [h_t_pred, y_t_pred],_ = theano.scan(fn=recurrent_fn2,
                                         outputs_info=[dict(initial=H_t, taps=[-1]), dict(initial=Y_t, taps=[-1])],
                                         n_steps=110)
    predict2 = theano.function(inputs=[H_t, Y_t],
                               outputs=[h_t_pred, y_t_pred])
    y_t_pred0 = np.reshape(y_t_val[-1], (1,1))
    h_t_pred0 = np.reshape(h_t_val[-1], (1,10))
    ...
    # Graph 1
    #plt.plot(seq_test[6,:,:])
    true_targets, = plt.plot(targets_test[6,:,:], label='y_t, true ouput')
