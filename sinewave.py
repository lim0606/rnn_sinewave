...
def prepare_data(seqs, labels, input_dim, output_dim, maxlen=None):
    ...
    x = numpy.zeros((maxlen-input_dim+1, n_samples, input_dim)).astype(theano.config.floatX)
    x_mask = numpy.zeros((maxlen-input_dim+1, n_samples)).astype(theano.config.floatX)
    y = numpy.zeros((maxlen-input_dim+1, n_samples, output_dim)).astype(theano.config.floatX)
    for idx, (s, l) in enumerate(zip(seqs, labels)):
        for i in range(0, lengths[idx]-input_dim+1):
            x[i, idx, :] = s[i:i+input_dim]
        #print "input_dim = %d" % input_dim 
        #print "length = %d" % lengths[idx]
        x_mask[:lengths[idx]-input_dim+1, idx] = 1.
        y[:lengths[idx]-input_dim+1, idx, :] = numpy.array(l[input_dim-1:lengths[idx]]).reshape([lengths[idx]-input_dim+1, output_dim])
