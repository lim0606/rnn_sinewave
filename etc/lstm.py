...
def lstm_layer(tparams, state_below, options, prefix='lstm', mask=None):
    ...
    rval, updates = theano.scan(_step,
                                ...
                                n_steps=nsteps)
    return rval[0] # rval[0] = h, rval[1] = c, this task requires h of lstm layer. 
...
def build_model(tparams, options):
    ...
    proj = get_layer(options['encoder'])[1](tparams, emb, options,    # lstm_layer
                                            prefix=options['encoder'],
                                            mask=mask)
...
def train_lstm(
    ...
    ydim = numpy.max(train[1]) + 1 # why should y dimension be one larger than existing y values? 
