...
def prepare_data(seqs, labels, maxlen=None):
    ...
    x = numpy.zeros((maxlen, n_samples)).astype('int64') # maxlen x n_samples
    x_mask = numpy.zeros((maxlen, n_samples)).astype(theano.config.floatX) # maxlen x n_samples
...
def load_data(path="imdb.pkl", n_words=100000, valid_portion=0.1, maxlen=None,
    ...
    print "len(train_set[0][0]) = %d, len(train_set[1][0]) = %d" % (len(train_set[0][0]), train_set[1][0])
    #print len(test_set)
    f.close()
    if maxlen:
        new_train_set_x = []
